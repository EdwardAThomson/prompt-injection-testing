...
from detectors import create_detectors, BaseDetector, DetectorResult
...
def main():
    ...
    try:
        ...
        # Check if multi-detector mode is active
        enabled_detectors = config.get('detectors', 'enabled')
        use_multi_detector = len(enabled_detectors) > 1 or enabled_detectors != ['llm_judge']

        if use_multi_detector:
            # --- Multi-detector pipeline ---
            print(f"\nDetectors enabled: {', '.join(enabled_detectors)}")
            detectors = create_detectors(config)

            expander = None
            if not config.get('run', 'no_expansion'):
            ...
            print("\nProcessing prompts...")
            results = process_prompts_multi(
                prompts, detectors, expander,
            ...
            report_generator.generate_multi_detector_reports(
                results,
            ...
            # --- Legacy single-detector pipeline (backward compatible) ---
            ...
            print("\nProcessing prompts...")
            results = process_prompts_legacy(
                prompts, safety_judge, expander,
                debug=config.get('run', 'debug'),
                output_dir=output_dir,
                config=config,
...
def process_prompts_multi(
    prompts: List[PromptData],
    detectors: Dict[str, BaseDetector],
    expander: Optional[PromptExpander],
    debug: bool = False,
    output_dir: str = None,
    config=None,
    original_command: str = None,
) -> List[Dict]:
    """Process prompts through all enabled detectors.

    Returns a list of dicts, one per prompt, each containing per-detector results.
    """
    results = []
    debug_dir = None
    ...
    for i, prompt_data in enumerate(prompts, 1):
        ...
        try:
            record = {
                ...
                "expanded_detector_results": {},
                "expand_latency_ms": 0,
            }

            # Run each detector on the original prompt
            for det_name, detector in detectors.items():
                print(f"  -> [{det_name}] classifying original...")
                result = detector.detect_timed(prompt_data.text)
                record["detector_results"][det_name] = result

            # Optionally expand, then re-run detectors
            if expander is not None:
                print(f"  -> Expanding prompt...")
                expanded_text, expand_latency = expander.expand(prompt_data.text)
                record["expanded_prompt"] = expanded_text
                record["expand_latency_ms"] = expand_latency

                for det_name, detector in detectors.items():
                    print(f"  -> [{det_name}] classifying expanded...")
                    result = detector.detect_timed(expanded_text)
                    record["expanded_detector_results"][det_name] = result

            results.append(record)

            # Print summary line
            labels = [f"{n}:{r.label}" for n, r in record["detector_results"].items()]
            print(f"  OK  {' | '.join(labels)}")

            # Debug logging
            if debug_dir:
                _write_multi_debug(debug_dir, prompt_data, record, config, original_command)
...
def _write_multi_debug(debug_dir, prompt_data, record, config, original_command):
    """Write debug JSON for a multi-detector run."""
    ...
    debug_data = {
        ...
        "execution_info": {
            ...
            "detectors": list(record["detector_results"].keys()),
        },
    }
    for det_name, dr in record["detector_results"].items():
        debug_data["detector_results"][det_name] = {
            "label": dr.label,
            "score": dr.score,
    ...
    if record["expanded_prompt"]:
        debug_data["expanded_prompt"] = record["expanded_prompt"]
        debug_data["expanded_detector_results"] = {}
        for det_name, dr in record["expanded_detector_results"].items():
            debug_data["expanded_detector_results"][det_name] = {
                "label": dr.label,
                "score": dr.score,
                "feedback": dr.feedback,
                "latency_ms": dr.latency_ms,
...
# ── Legacy single-detector pipeline (unchanged behavior) ─────────────

def process_prompts_legacy(prompts: List[PromptData], safety_judge: SafetyJudge,
    ...
    """Process all prompts through the safety and expansion pipeline (original behavior)."""
    results = []
        ...
        try:
            ...
            if debug_dir:
                ...
                with open(debug_file, 'w') as f:
                    ...
                    # Add system prompts to debug output
            ...
            if no_expansion_mode:
                print(f"  OK  {orig_label} ({original_judgement.score:.2f})")
            else:
                exp_label = expanded_judgement.label
                delta = result.score_delta
                change_indicator = " !!" if result.label_changed else ""
                print(f"  OK  {orig_label}->{exp_label} (d{delta:+.2f}){change_indicator}")
